...
export class ObjectHelper {
    ...
    static setEditFormValidators(fieldSettings: IFormItemSetting[], formGroup: UntypedFormGroup)
    {
        fieldSettings.forEach(field =>
        {
            ...
            {
                ObjectHelper.setEditFormValidators(<IFormItemSetting[]>(<IGroupBoxSettings>field).fieldSettings, formGroup);
            }
            else if (field.abstractControlType === abstractControlKind.formGroup)
            {
            ...
            else if (field.abstractControlType === abstractControlKind.formGroupArray)
            {
                ...
                formArray.controls.forEach(control => {
                    ...
                    {//Only Supporting formGroups as elements of FormArray
                        ObjectHelper.setEditFormValidators(<IFormItemSetting[]>(<IFormGroupArraySettings>field).fieldSettings, <UntypedFormGroup>control);
                    }
                })
            }
        });
    ...
    static getValidatorFunction(validator: IValidatorDescription): any {
        let validatorClass = ValidatorsManager.GetValidatorClass(validator.className) as Record<string, any> | undefined;
        if (!validatorClass) {
            return null;
        }

        let validatorFunction = validatorClass[validator.functionName];
